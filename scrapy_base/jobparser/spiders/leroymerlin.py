...
class LeroymerlinSpider(scrapy.Spider):
    ...
    def __init__(self, text):
        self.start_urls = [f'https://leroymerlin.ru/search/?q=обои']

    def parse(self, response: HtmlResponse):
        links_on_product = response.xpath("//div[@data-qa-product]//a[@data-qa='product-name']/@href").extract()

        for link in links_on_product:
            if 'product' in link:
                yield response.follow(link, callback=self.parse_product)

        next_page = response.xpath("//div[@role='navigation']/a/@href").extract_first()
